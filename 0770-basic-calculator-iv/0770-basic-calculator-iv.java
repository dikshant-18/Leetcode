class Poly {
    ...
    static String merge(final String a, final String b) {
    ...
    String concat(final String term) {
        if (term.equals("1")) return String.valueOf(terms.get(term));
...
class Solution {
    ...
    private List<String> getTokens(final String s) {
        ...
        for (int j = 0; j < s.length(); ++j) {
            if (s.charAt(j) == ' ') {
                if (i < j) tokens.add(s.substring(i, j));
                i = j + 1;
    ...
    private Poly evaluate(List<String> postfix) {
        ...
        for (final String token : postfix) {
            if (isOperator(token)) {
                ...
                if (token.equals("+")) polys.add(a.add(b));
                else if (token.equals("-")) polys.add(a.minus(b));
                else polys.add(a.mult(b));
